...
def get_device():
    if torch.cuda.is_available():
        ...
        return torch.device('mps')   # Use MPS if CUDA is not available but MPS is
    else:
        return torch.device('cpu')   # Fallback to CPU if neither CUDA nor MPS is available
...
model = GPT(GPTConfig(vocab_size=50304)) # 50257 + 47 extra tokens, so that the number is "nicer" (more power-two-ish)
model.to(device)
torch.compile(model)
